...
const DealsModal: React.FC<Props> = ({ isOpen, closeModal,process,deleteId ,selectedRow}) => {
  ...
  const { trigger: triggerDelete, data: dataDelete, error: errorDelete, isMutating: isMutatingDelete } = useSWRMutation(
    "/api/OrderAdmin/Delete",
    Delete.user
  );
  const { trigger: triggerEdit, data: dataEdit, error: errorEdit, isMutating: isMutatingEdit } = useSWRMutation(
    "/api/OrderAdmin/Update",
    EditDeal.user
  );
  const mutateData = async () => {
    const { data, error } = await fetch('/api/OrderAdmin/GetAll').then(res => res.json());
    if (error) {
      console.log(error);
    } else {
  ...
  const handleSubmit = async (values: Values) => {
    ...
    const parsedValues = {
      ...values,
      actualDeadline: new Date(values.actualDeadline).toLocaleDateString('en-GB', { day: '2-digit', month: '2-digit', year: 'numeric' }).split('/').reverse().join('-'),
      normativeDeadline: new Date(values.normativeDeadline).toLocaleDateString('en-GB', { day: '2-digit', month: '2-digit', year: 'numeric' }).split('/').reverse().join('-'),
